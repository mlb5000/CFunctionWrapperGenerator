...
function_file           [Required] Path to a file containing a list of C function names to wrap.  This file contains a number of
                        lines in the format:
...
The cppcheck Parser is not perfect and may choke on some files.  If you see an error like: 'I did my best, but I can go no further. Hopefully the collected ASTs are sufficient for your needs' it means that the parser failed on something, but that some symbols were collected.